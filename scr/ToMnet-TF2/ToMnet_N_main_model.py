...
import ToMnet_N_CharNet as cn
import ToMnet_N_PredNet as pn
...
class ToMnet_N(Model):
    ...
    def __init__(self):
        ...
        self.pred_net = pn.PredNet(n=self.NUM_RESIDUAL_BLOCKS)

        # Set compilers / savers / loggers / callbacks


    def call(self, inputs):
        input_trajectory = inputs["trajectory"]
        input_current_state = inputs["current_state"]

        e_char = self.char_net(input_trajectory)

        mix_input = {
            "e_char": e_char,
            "input_current_state": input_current_state
        }

        pred = self.pred_net(mix_input)
...
class Handler_ToMnet_N():
  ...
  # 11 = 1 (obstacle) + 1 (agent initial position) + 4 (targets) + 5 (actions: up/down/left/right/goal)
  MAZE_DEPTH_TRAJECTORY = 11

  # MAZE_QUERY_STATE_DEPTH = number of channels of each query state (current state)
  ...
  def __init__(
    ...
    # Set up constants
    # --------------------------------------------------------------
    path_ckpt = os.path.join(self.path_ckpt,
                             self.TRAINING_VERSION,
                             args.subj_name)

    path_train = os.path.join(self.path_train,
                              self.TRAINING_VERSION,
                              args.subj_name)
    ...
    path_ckpt = os.path.join(path_ckpt, 'logs', 'model.ckpt')
    ...
    # self.batch_generator = bg.BatchGenerator()    # Will be replaced with Keras Batch Generator

    # --------------------------------------------------------------
    # Load Data: Parse the trajectory data and labels
    ...
    self.all_files_traj, self.train_files_traj, \
    self.vali_files_traj, self.test_files_traj = \
    ...
    self.all_files_query_state, self.train_files_query_state, \
    self.vali_files_query_state, self.test_files_query_state = \
      data_handler.parse_whole_data_set(dir,
                                        mode=args.mode,
                                        shuf=args.shuffle,
                                        subset_size=self.SUBSET_SIZE,
  ...
  def train(self):
    ...
    inputs = {
        "trajectory": WHOLE_TRAJECTORIES_DATA,
        "current_state": WHOLE_STATES_DATA
    }
    input_trajectory = inputs["trajectory"]
    input_current_state = inputs["current_state"]
